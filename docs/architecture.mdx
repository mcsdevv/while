...
This document explains the technical architecture of Notion-Google Calendar Sync.
...
| `/api/cron/sync-notion` | GET | Poll Notion for changes |
...
sequenceDiagram
    ...
    Cron->>Notion: Query database (cursor)
    Notion-->>Cron: Changed pages
    Cron->>Sync: Process changes
    Sync->>GCal: Create/Update/Delete events
    GCal-->>Sync: Confirmation
```

**Timing:** Every 2 minutes (polling)

1. Cron job triggers every 2 minutes
2. App queries Notion for pages changed since last sync
3. Sync engine processes each change
4. Corresponding Google Calendar events are updated
...
|-----|----------|---------|
| `sync-notion` | Every 2 min | Poll Notion for changes |
| `renew-webhook` | Daily | Renew Google Calendar webhook |
