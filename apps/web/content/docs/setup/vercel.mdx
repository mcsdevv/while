...
<FAQAccordion>
  ...
  <FAQAccordionItem title="Redis Connection Failed">
    - Redis is auto-provisioned via the deploy button. If missing, redeploy using the deploy button above.
    - Verify `KV_REST_API_URL` and `KV_REST_API_TOKEN` are set in project settings
    - Try disconnecting and reconnecting the Upstash integration from the Vercel Storage tab
  </FAQAccordionItem>
</FAQAccordion>
