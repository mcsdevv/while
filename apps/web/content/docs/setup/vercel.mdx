...
<EnvVarsTable vars={[
  ...
  { name: "KV_REST_API_URL", description: "Upstash Redis REST endpoint", howToGetLink: { href: "#add-redis-storage", label: "Add via Vercel Storage" }, hideInput: true },
  { name: "KV_REST_API_TOKEN", description: "Upstash Redis auth token", howToGetLink: { href: "#add-redis-storage", label: "Add via Vercel Storage" }, hideInput: true },
]} />

<Callout type="info">
Redis storage (`KV_REST_API_URL` and `KV_REST_API_TOKEN`) is automatically configured when you add the Upstash integration from the Vercel Marketplace.
</Callout>
...
5. The environment variables (`KV_REST_API_URL` and `KV_REST_API_TOKEN`) are automatically added to your project
