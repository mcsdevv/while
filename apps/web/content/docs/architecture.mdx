...
```mermaid
graph LR
    subgraph deploy ["Your Vercel Deployment"]
        direction TB
        App([Next.js App])
        API([API Routes])
        Cron([Cron Jobs])
...
| `/api/sync/backfill/fields` | GET/POST/DELETE | Populate newly-enabled fields on existing Notion pages |
...
| `sync:backfill:progress` | Backfill operation status and progress |
...
A [daily cron job](https://www.while.so/docs/architecture#cron-jobs) runs as fallback to catch any missed webhooks. This is highly unlikely—Notion webhooks are reliable—but the cron ensures data consistency if network issues or transient failures occur.
...
```mermaid
flowchart LR
    A([Change Detected]) --> B{Both systems<br/>modified?}
    B -->|No| C([Apply change])
    B -->|Yes| D{Compare<br/>timestamps}
    D --> E([Apply most recent])
```
